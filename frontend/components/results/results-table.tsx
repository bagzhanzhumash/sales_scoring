...
export function ResultsTable({ results, selectedResults, onSelectResult, onSelectAll, onRowClick }: ResultsTableProps) {
      ...
      <div className="divide-y divide-slate-200 dark:divide-slate-700">
        {results.map((result, index) => (
          <motion.div
            ...
            className={`grid grid-cols-12 gap-4 px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer group ${
              selectedResults.includes(result.id) ? "bg-blue-50 dark:bg-blue-900/20" : ""
            }`}
          ...
          >
            ...
            <div className="col-span-1 flex items-center">
              <input
                ...
                onChange={(e) => {
                  e.stopPropagation()
                  onSelectResult(result.id, e.target.checked)
                }}
                className="w-4 h-4 text-blue-600 rounded"
              />
            </div>
            ...
            <div className="col-span-3 flex items-center space-x-2">
              ...
              <span className="font-medium text-slate-900 dark:text-white truncate" title={result.file_name}>
                {result.file_name}
              </span>
            </div>
              ...
              {editingScore === result.id ? (
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      handleEditScore(result.id, result.overall_score)
                    }}
            ...
            {/* Date */}
            <div className="col-span-1 flex items-center">
              <span className="text-sm text-slate-600 dark:text-slate-400">{formatDate(result.created_at)}</span>
            </div>
